...
const Navbar = () => {
  ...
  const { location, setLocation, locationShow, setLocationShow } = useLocationContext();
  ...
  return (
    <header className="sticky top-0 z-50 font-poppins bg-white text-2xl h-20 shadow-md">
      <nav className="container mx-auto px-6 flex justify-between items-center h-full" aria-label="Global">
        {/* Logo Section */}
        ...
        <div className={`items-center ${locationSet ? "justify-center" : "justify-end space-x-10"} hidden lg:flex gap-3 md:gap-5 lg:gap-x-10 flex-grow`}>
          {navigation.map((item) => (
        ...
        {/* Location Display */}
        {locationSet && (
          <div className="ml-6 cursor-pointer hidden lg:block border-blue-500 rounded-full bg-violet-400 py-2 text-lg px-2 md:px-5" onClick={() => setLocationShow(true)}>
            <img src={l_icon} alt="Location Icon" className="h-6 w-6 inline-block" />
        ...
        <div className="lg:hidden flex items-center">
          <button type="button" className="inline-flex items-center justify-center p-2" onClick={() => setMobileMenuOpen(true)}>
      ...
      <Dialog as="div" open={mobileMenuOpen} onClose={setMobileMenuOpen}>
        <DialogPanel className="fixed inset-0 z-50 overflow-y-auto bg-gray-100 lg:hidden top-0 right-0 w-full max-w-xs p-6">
          <div className="flex items-center justify-between">
            {/* Close Button */}
            <button type="button" className="rounded-md p-2.5" onClick={() => setMobileMenuOpen(false)}>
          ...
          <div className="mt-6">
            ...
            {locationSet && (
              <div className="rounded-full mt-3 text-xl font-normal cursor-pointer bg-violet-400 py-3 px-3" onClick={() => setLocation("Default")}>
                <img src={l_icon} alt="Location Icon" className="h-6 w-6 inline-block" />
                {capitalizeFirstLetter(location)}
              </div>
            )}
